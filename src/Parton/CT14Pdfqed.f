c-- JC, 7/21/16: lightly edited for OMP and disambiguation from original CT14Pdf 
C============================================================================
...
C   (10) 62 CT14QED sets (LO QED, NLO QCD)        [4]
C        31 each for proton and neutron
...
C
C   The function CT14Alphas_qed (Q) 
...
C     is included in the input .pds file. The function CT14Alphas_qed
...
C     Call CT14GetPars_qed( xmin,Qini,Qmax,Nloops,Nfl,IHDN,Lpho),
C   which returns
...
C===========================================================================
      ...
      Function CT14Pdfqed (Iparton, X, Q)
      Implicit Double Precision (A-H,O-Z)
      Logical Warn, Lpho, Lbig, Lnp
      integer isetch, ipdsformat
      Common
     > / CtqPar2_qed / Nx, Nt, NfMx, MxVal, Ihadron
     > / QCDtbl_qed /  AlfaQ, Qalfa, Ipk, Iorder, Nfl !for external use
     >  /Setchange_qed / Isetch, ipdsset, ipdsformat
      Common / Photon_qed / Lpho
...
!$omp threadprivate(Warn,Qsml)
      ...
      if (ipdsset.ne.1) 
     >  STOP 'CT14Pdfqed: the PDF table was not initialized'

      If (X .lt. 0d0 .or. X .gt. 1D0) Then
        Print *, 'X out of range in CT14Pdfqed: ', X
        CT14Pdfqed = 0D0
        Return
      Endif

      If (Q .lt. Qsml) Then
        Print *, 'Q out of range in CT14Pdfqed: ', Q
        Stop
      Endif
...
C        print a warning for calling extra flavor
          Warn = .false.
          Print *, 'Warning: Iparton out of range in CT14Pdfqed! '
          Print *, 'Iparton, MxFlvN0: ', Iparton, NfMx
        Endif
        CT14Pdfqed = 0D0
      else

        CT14Pdfqed = PartonX12_qed (Iparton, X, Q)
        if (CT14Pdfqed.lt.0D0) CT14Pdfqed = 0D0
...
C                             ********************
      ...
      Subroutine SetCT14_qed(Tablefile)
      Implicit Double Precision (A-H,O-Z)
      character*72 filename,checkpath
      Character Tablefile*40
      Common /Setchange_qed / Isetch, ipdsset, ipdsformat
...
C                             ********************
      ...
      subroutine CT14GetPars_qed(xmin,Qini,Qmax,Nloops,Nfl,IHDN,Lphoton)
...
c               otherwise .false
      ...
      common / XQrange_qed / Qini0, Qmax0, Xmin0
      common / QCDtbl_qed /  AlfaQ, Qalfa, Ipk, Iorder, Nfl0
      common / CtqPar2_qed / Nx, Nt, NfMx, MxVal, Ihadron
      common / Photon_qed / Lpho
...
c      PARAMETER (MXPQX = (MXF+1+MaxVal) * MXQ * MXX)
      ...
      Common
     > / CtqPar1_qed / qBase,XV(0:MXX), TV(0:MXQ),AlsCTEQ(0:mxq)
     > / CtqPar2_qed / Nx, Nt, NfMx, MxVal, Ihadron
     > / CtqPar3_qed / UPD(0:mxx-1,0:mxq-1,-(mxf+1):(MaxVal))
     > / XQrange_qed / Qini, Qmax, Xmin
     > /Setchange_qed / Isetch, ipdsset, ipdsformat
      ...
      if (ipdsset.ne.1) 
     >  STOP 'CT14Alphas_qed: the PDF table was not initialized'

      
      if (ipdsformat.lt.11) then
        print *
        print *, 
     >    'STOP in CT14Alphas_qed: the PDF table file has an older format'
        print *,
     >    'and does not include the table of QCD coupling values.'
        print *, 
     >    'You can still compute the PDFs, but do not call'
        print *,
     >    'the CT14Alphas_qed function for the interpolation of alpha_s.'
        stop
      endif
...
C                                       ********************
      ...
      function CT14Mass_qed(i)
c     Returns the value of the quark mass for the i-th flavor 
c     The flavors are:
c     1  2  3  4  5  6
c     u  d  s  c  b  t
      implicit none
      double precision CT14Mass_qed, Amass
     ...
     >  / Masstbl_qed / Amass(6)
      ...
      if (ipdsset.ne.1) 
     >  STOP 'CT14Mass_qed: the PDF table was not initialized'

      CT14Mass_qed = Amass(i)

      return 
      end


      Subroutine Readpds0_qed (Nu)
      Implicit Double Precision (A-H,O-Z)
      Character Line*80
      integer ipdsformat
Carl  The following match those in mEvlPac.F for v05t:
...
c check that qBase is consistent with the definition of Tv(0:nQ) for 2 values of Qv
      ...
      if (abs(qbase1-qbase2).gt.1e-5) then
        print *, 'Readpds0_qed: something wrong with qbase'
...
C                        ****************************
      ...
      Function PartonX12_qed (IPRTN, XX, QQ)
...
C
      If     (JLx .LE. -1) Then
        Print '(A,1pE12.4)','Severe error: x <= 0 in PartonX12_qed! x = ',x
        Stop
...
C                                      Keep at least 4 points >= Jx
         Jx = JLx - 2
      Else
        Print '(A,1pE12.4)','Severe error: x > 1 in PartonX12_qed! x = ',x
        Stop
      Endif
